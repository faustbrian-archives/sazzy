    ...
    <div class="sm:p-6 md:flex-row flex flex-col items-center justify-between px-4 py-5">
        <div>
            ...
            <div class="max-w-xl2 mt-2 text-sm leading-5 text-gray-600">
                <p>This will delete all the teams you're the owner of and delete your account. There's no
                    turning back.</p>
            </div>
        </div>
        <div class="md:w-auto w-full">
            <div x-data="{ open: false }" @keydown.window.escape="open = false">
                <button @click="open = true" type="button" class="md:mt-0 md:w-auto focus:outline-none sm:text-sm sm:leading-5 focus:border-red-300 focus:shadow-outline-red active:bg-red-200 hover:bg-red-50 relative inline-flex items-center justify-center w-full px-4 py-2 mt-4 font-medium text-red-700 transition duration-150 ease-in-out bg-red-100 border border-transparent rounded-md">
                    ...
                    <div @click.away="open = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="sm:max-w-lg sm:w-full sm:p-6 px-4 pt-5 pb-4 overflow-hidden transition-all transform bg-white rounded-lg shadow-xl">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="sm:mt-0 sm:ml-4 sm:text-left mt-3 text-center">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm leading-5 text-gray-500">Are you sure you want to deactivate your account? All of your data will be permanantly removed from our servers, including all the teams you own and the sites contained on those teams. This action cannot be undone.</p>
                                </div>
                            </div>
                        </div>
                        <div class="sm:mt-4 sm:flex sm:flex-row-reverse mt-5">
                            <span class="sm:ml-3 sm:w-auto flex w-full rounded-md shadow-sm">
                                <button wire:click="deleteAccount" type="button" class="hover:bg-red-500 focus:outline-none focus:border-red-700 focus:shadow-outline-red sm:text-sm sm:leading-5 inline-flex justify-center w-full px-4 py-2 text-base font-medium leading-6 text-white transition duration-150 ease-in-out bg-red-600 border border-transparent rounded-md shadow-sm">
                                    Delete
                                </button>
                            </span>
